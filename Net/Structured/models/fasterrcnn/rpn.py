...
class RPN:
    ...
    def predict(self):
        ...
        self.rpn_conv_feature       = convolution(
            self.conv_feats, 3, 3, 512, 1, 1, 'rpn_conv', stddev=0.01, reg=self.reg,  group_id=1
        ...
        self.rpn_cls_score_original  = convolution(
            self.rpn_feature, 1, 1, 2 * self.anchors_count, 1, 1, 'rpn_cls', stddev=0.01, reg=self.reg, group_id=1
        ...
        self.rpn_bbox_pred_original  = convolution(
            self.rpn_feature, 1, 1, 4 * self.anchors_count, 1, 1, 'rpn_regs', stddev=0.001, reg=self.reg, group_id=1
    ...
    def loss(self, rpn_cls_score, rpn_cls_target, rpn_bbox_pred, rpn_bbox_target):
        ...
        with tf.variable_scope('RPNLoss'):
            ...
            cross_entropy_per_anchor = tf.nn.softmax_cross_entropy_with_logits_v2(
