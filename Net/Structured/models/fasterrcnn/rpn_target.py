...
class RPNTarget:
    ...
    def __init__(self, config, num_anchors):
        self.config                 = config
        self.num_anchors            = num_anchors

        # Margin to crop proposals to close to the border.
        self.allowed_border         = self.config.allowed_border
        ...
        # `positive_overlap`.
        self.positive_overlap       = self.config.foreground_threshold
        # We set anchors as negative when the IoU is less than
        # `negative_overlap`.
        self.negative_overlap       = self.config.background_threshold_high
        # Fraction of the batch to be foreground labeled anchors.
        self.foreground_fraction    = self.config.foreground_fraction
        self.minibatch_size         = self.config.minibatch_size


    def get_targets(self, all_anchors, gt_boxes, img_shape):
        """
        ...
        anchor_filter = tf.logical_and(
            tf.logical_and(
                tf.greater_equal(x_min_anchor, -self.allowed_border),
                tf.greater_equal(y_min_anchor, -self.allowed_border)
            ),
            tf.logical_and(
                tf.less(x_max_anchor, img_shape[1] + self.allowed_border),
                tf.less(y_max_anchor, img_shape[0] + self.allowed_border)
            )
        )
        ...
        if not self.clobber_positives:
            ...
            negative_overlap_nonzero = tf.less(
                max_overlaps, self.negative_overlap)
        ...
        positive_overlap_inds = tf.greater_equal(
            max_overlaps, self.positive_overlap)
        ...
        num_fg = tf.to_int32(self.foreground_fraction * self.minibatch_size)
        ...
        num_bg = tf.to_int32(self.minibatch_size - fg_inds_size)
