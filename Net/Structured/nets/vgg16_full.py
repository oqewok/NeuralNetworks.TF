...
def build_basic_vgg16(config):
    ...
    gt_boxes = tf.placeholder(tf.float32, [None, 5], name="gt_boxes")

    bbox = gt_boxes[:, 0:4]
    labels = tf.to_int32(gt_boxes[:, 4])
    ...
    #
    # outputs = tf.matmul(h_fc2, W_out) + b_out
    '''
    conv1_1_feats = convolution(inputs, 3, 3, 32, 2, 2, 'conv1_1', stddev=0.1)
    conv1_2_feats = batch_norm(conv1_1_feats, 'bn1_2', is_train, bn, 'relu')
    pool1_feats = max_pool(conv1_2_feats, 2, 2, 2, 2, 'pool1')

    conv2_1_feats = convolution(pool1_feats, 3, 3, 64, 1, 1, 'conv2_1', stddev=0.1)
    conv2_2_feats = nonlinear(conv2_1_feats, 'relu')
    pool2_feats = max_pool(conv2_2_feats, 2, 2, 2, 2, 'pool2')

    conv3_1_feats = convolution(pool2_feats, 3, 3, 128, 1, 1, 'conv3_1', stddev=0.1)
    conv3_2_feats = nonlinear(conv3_1_feats, 'relu')
    pool3_feats = max_pool(conv3_2_feats, 2, 2, 2, 2, 'pool3')

    h_pool3_flat = tf.reshape(pool3_feats, [-1, 15 * 19 * 128])

    fc6_feats = fully_connected(h_pool3_flat, 1000, 'fc1')
    fc6_feats = nonlinear(fc6_feats, 'relu')
    fc6_feats = dropout(fc6_feats, dropout_prob, is_train)

    fc7_feats = fully_connected(fc6_feats, 500, 'fc2')
    fc7_feats = nonlinear(fc7_feats, 'relu')

    bbox_reg = fully_connected(fc7_feats, 4, 'bboxes')

    cls_score = fully_connected(fc7_feats, num_classes, 'cls')
    cls_probs = tf.nn.softmax(cls_score)
    '''
    conv1_1_feats = convolution(inputs, 3, 3, 64, 1, 1, 'conv1_1', stddev=0.01)
    conv1_1_feats = batch_norm(conv1_1_feats, 'bn1_1', is_train, bn, 'relu')
    conv1_2_feats = convolution(conv1_1_feats, 3, 3, 64, 1, 1, 'conv1_2', stddev=0.01)
    conv1_2_feats = batch_norm(conv1_2_feats, 'bn1_2', is_train, bn, 'relu')
    pool1_feats   = max_pool(conv1_2_feats, 2, 2, 2, 2, 'pool1')
    #
    conv2_1_feats = convolution(pool1_feats, 3, 3, 128, 1, 1, 'conv2_1', stddev=0.01)
    conv2_1_feats = batch_norm(conv2_1_feats, 'bn2_1', is_train, bn, 'relu')
    conv2_2_feats = convolution(conv2_1_feats, 3, 3, 128, 1, 1, 'conv2_2', stddev=0.01)
    conv2_2_feats = batch_norm(conv2_2_feats, 'bn2_2', is_train, bn, 'relu')
    pool2_feats   = max_pool(conv2_2_feats, 2, 2, 2, 2, 'pool2')
    #
    conv3_1_feats = convolution(pool2_feats, 3, 3, 256, 1, 1, 'conv3_1', stddev=0.01)
    conv3_1_feats = batch_norm(conv3_1_feats, 'bn3_1', is_train, bn, 'relu')
    conv3_2_feats = convolution(conv3_1_feats, 3, 3, 256, 1, 1, 'conv3_2', stddev=0.01)
    conv3_2_feats = batch_norm(conv3_2_feats, 'bn3_2', is_train, bn, 'relu')
    conv3_3_feats = convolution(conv3_2_feats, 3, 3, 256, 1, 1, 'conv3_3', stddev=0.01)
    conv3_3_feats = batch_norm(conv3_3_feats, 'bn3_3', is_train, bn, 'relu')
    pool3_feats   = max_pool(conv3_3_feats, 2, 2, 2, 2, 'pool3')
    #
    conv4_1_feats = convolution(pool3_feats, 3, 3, 512, 1, 1, 'conv4_1', stddev=0.01)
    conv4_1_feats = batch_norm(conv4_1_feats, 'bn4_1', is_train, bn, 'relu')
    conv4_2_feats = convolution(conv4_1_feats, 3, 3, 512, 1, 1, 'conv4_2', stddev=0.01)
    conv4_2_feats = batch_norm(conv4_2_feats, 'bn4_2', is_train, bn, 'relu')
    conv4_3_feats = convolution(conv4_2_feats, 3, 3, 512, 1, 1, 'conv4_3', stddev=0.01)
    conv4_3_feats = batch_norm(conv4_3_feats, 'bn4_3', is_train, bn, 'relu')
    pool4_feats   = max_pool(conv4_3_feats, 2, 2, 2, 2, 'pool4')

    conv5_1_feats = convolution(pool4_feats, 3, 3, 512, 1, 1, 'conv5_1', stddev=0.01)
    conv5_1_feats = batch_norm(conv5_1_feats, 'bn5_1', is_train, bn, 'relu')
    conv5_2_feats = convolution(conv5_1_feats, 3, 3, 512, 1, 1, 'conv5_2', stddev=0.01)
    conv5_2_feats = batch_norm(conv5_2_feats, 'bn5_2', is_train, bn, 'relu')
    conv5_3_feats = convolution(conv5_2_feats, 3, 3, 512, 1, 1, 'conv5_3', stddev=0.01)
    conv5_3_feats = batch_norm(conv5_3_feats, 'bn5_3', is_train, bn, 'relu')
    pool5 = max_pool(conv5_3_feats, 2, 2, 2, 2, 'pool5')

    pool5_feats_flat = tf.reshape(pool5, shape=[-1, 7 * 7 * 512])

    fc6_feats = fully_connected(pool5_feats_flat, 1000, 'fc6', stddev=0.01)
    fc6_feats = nonlinear(fc6_feats, 'relu')
    fc6_feats = dropout(fc6_feats, dropout_prob, is_train)

    fc7_feats = fully_connected(fc6_feats, 500, 'fc7', stddev=0.01)
    fc7_feats = nonlinear(fc7_feats, 'relu')
    #fc7_feats = dropout(fc7_feats, dropout_prob, is_train)
    #
    cls_scores = fully_connected(fc7_feats, num_classes, 'fc_cls', stddev=0.01)
    ...
    return inputs, cls_probs, labels, gt_boxes, is_train
