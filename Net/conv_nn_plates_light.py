...
conv_kernels = {
    'h_conv1': {'size': (7, 7), 'stride': (2, 2)},
    'h_conv2': {'size': (3, 3), 'stride': (2, 2)},
    'h_conv3': {'size': (3, 3), 'stride': (1, 1)},
    'h_conv4': {'size': (8, 10), 'stride': (1, 1)},
}
...
weights = {
    ...
    'W_fc1_c': weight_variable([1 * 1 * n_hidden_4, n_fc_1], 'W_fc1_c'),
    'W_fc2_c': weight_variable([n_fc_1, n_fc_2], 'W_fc2_c'),
    'W_fc1_r': weight_variable([1 * 1 * n_hidden_4, n_fc_1], 'W_fc1_r'),
...
def conv_layers():
    ...
    # First layer (64 kernel conv 11x11, max_pool_2x2). Result: 59x79x64
    W_conv1 = weights['h1']
    ...
    # Second layer (128 kernels conv 5x5, max_pool_2x2). Result: 20x15x128
    W_conv2 = weights['h2']
    ...
    # Third layer (256 kernels conv 3x3). Result: 10x8x256
    W_conv3 = weights['h3']
    ...
    # Fourth layer (512 kernels conv 3x3). Result: 1x1x1024
    W_conv4 = weights['h4']
    b_conv4 = biases['b_conv4']

    h_conv4 = tf.nn.relu(tf.add(
        conv2d(h_pool3, W_conv4, stride=conv_kernels['h_conv4']['stride'], padding='VALID', name='h_conv4'), b_conv4,
        name="h_conv4_b"), name='h_conv4_relu')


    return x, h_conv4  # , [W_conv1, b_conv1,
    #  W_conv2, b_conv2,
    #  W_conv3, b_conv3]
...
def get_classification_model():
    ...
    conv_layer_flat = tf.reshape(conv_layer, [-1, 1 * 1 * n_hidden_4])
